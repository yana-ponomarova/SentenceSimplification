...
public class DefaultPropositionGenerator extends PropositionGenerator {
	public DefaultPropositionGenerator(SentenseSimplification SentenseSimplification) {
		super(SentenseSimplification);
	}
	...
	public void generate(List<Proposition> result, Clause clause,
		...
		for (Integer index : sortedIndexes) {
			...
			if (include.get(index)) {
				for(Proposition p: propositions) {
					p.constituents.add( generate(clause, index) );
					if (clause.getFlag(index, SentenseSimplification.options).equals(Flag.OPTIONAL)) {
		...
		if (!SentenseSimplification.options.nary ) {
			for(Proposition p: propositions) {
				p.optional.clear();
